...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { data: goal, error: goalError } = await supabase
      .from("goals")
      .select("id,user_id,title,stake,stake_currency,deadline,status,is_private,judge_user_id,payment_intent_id,payment_method_id,stripe_customer_id,payment_status,stake_recipient_user_id,stake_charity_id")
      .eq("id", token.goal_id)
      .single();
